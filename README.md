...
|---|---|---|
| 🎯 Координатор | `@coordintor_bot` | Управляет маршрутом обсуждения, следит за финализацией |
| 🔍 Исследователь | `@Researcher_bot` | Собирает факты, контекст, ограничения |
| 🏗️ Архитектор | `@Architect_bot` | Проектирует архитектуру, компоненты, API, инфраструктуру |
| ⚡ Исполнитель | `@executor_bot` | Делает практический результат: код, файлы, планы |
| 🧪 QA | `@Qa_bot` | Проверяет результат, edge cases, тест-кейсы |
| 🧐 Критик | `@critical_bot` | Ищет слабые места, риски, противоречия |
